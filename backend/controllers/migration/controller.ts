  ...
  getTestCaseImportTemplate() {
    const template = {
      columns: [
        ...
        // Additional fields
        { name: 'RTC-ID', required: false, description: 'RTC identifier for traceability' },
        { name: 'Flow-ID', required: false, description: 'Flow identifier for test flow grouping' },
        { name: 'Layer', required: false, description: 'Test layer: Smoke / Core / Extended' },
        { name: 'Target', required: false, description: 'Test target: API / Screen (画面)' },
        { name: 'Test Type', required: false, description: 'Test type: 正常系 / 異常系 / 非機能 / 初期確認 / データ整合性確認 / 状態遷移確認 / 運用確認 / 障害時確認 / 回帰' },
        { name: 'Evidence', required: false, description: 'Document reference or evidence source' },
